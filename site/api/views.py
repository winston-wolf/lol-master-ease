...
def db_get_match_ids(region, summoner, begin_index, end_index, champion_ids=[]):
    ...
    """.format(
        ...
        offset=begin_index,
        champion_filter_sql=champion_filter_sql,
...
def api_pull_match_history(region, summoner, begin_index, champion_ids=[]):
    ...
    response = request(API_URL_MATCH_LIST, region, summonerId=summoner['id'], beginIndex=begin_index, endIndex=end_index, championIds=','.join([str(champion_id) for champion_id in champion_ids]))
...
class Matches(restful.Resource):
    def get(self):
        ...
        """.format(
            database.escape(region),
            ','.join([str(match_id) for match_id in match_ids]),
            summoner['id'],
